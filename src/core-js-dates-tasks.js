...
function getTime(date) {
  const day = new Date(date);
  return day.toTimeString().slice(0, 8);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const timeDiff = dayEnd.getTime() - dayStart.getTime() + 86400000;
...
function isDateInPeriod(date, period) {
  ...
  return dayStart <= day && day <= dayEnd;
}
...
function formatDate(date) {
  const day = new Date(date);
  const answer = day.toLocaleString('US', { timeZone: 'UTC' });
  return answer;
}
...
function getWeekNumberByDate(date) {
  ...
  const days = Math.floor((day - startDate) / (24 * 3600 * 1000));
  const weekNumber = Math.ceil((days + (startDate.getDay() || 7)) / 7);
